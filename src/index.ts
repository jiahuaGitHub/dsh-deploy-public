...
import {
  ...
  requestDeviceCode, finishGhAuth, ghAuthed, publishToGithub, killPidTree,
  type DeviceFlow,
} from './engine.js'
...
let pendingFlow: DeviceFlow | undefined

function tailLogs(logs: string[], n = 40): string {
  return logs.slice(-n).join('\n')
}

function remember(kind: Running['kind'], child: ChildProcess, extra: { port?: number; projectDir?: string } = {}): Running {
  const id = `${kind}-${++seq}`
    ...
    async execute(args: Record<string, unknown>) {
      ...
      if (mode === 'permanent') {
        ...
        if (pendingFlow) {
          ...
          if (!fin.ok) return JSON.stringify({ status: 'error', step: 'finish-auth', detail: fin.detail })
        }
