...
import { existsSync, readFileSync, writeFileSync, rmSync, mkdirSync, appendFileSync } from 'node:fs';
...
export function runArgs(cmd, args, opts = {}) {
    return new Promise((resolve) => {
        ...
        const t = opts.timeoutMs ? setTimeout(() => { try {
            child.kill();
        }
        catch { /* noop */ } }, opts.timeoutMs) : undefined;
        child.on('close', (code) => { if (t)
            clearTimeout(t); resolve({ code, stdout: out, stderr: err }); });
...
export async function verifyPublic(url) {
    ...
    try {
        ...
        return {
            ok,
            detail: `本地 DNS 失败(${lastErr?.cause ? String(lastErr.cause.code) : lastErr?.message})；8.8.8.8 直连验证 ${ok ? '正常' : '失败'}@${ip}`,
            directOk: ok,
            ip,
        };
    }
...
export function startDeviceFlow() {
    const handle = { userCode: '', verifyUrl: '', promise: Promise.resolve('error') };
    const promise = (async () => {
        try {
...
export async function finishGhAuth() {
...
export async function ensureGhAuthBlocking() {
    ...
    if ('error' in handle)
        return { ok: false, detail: handle.error };
...
// ---------------------------------------------------------------------------
export async function publishToGithub(projectDir, repoName, startCommand) {
    ...
    if (existsSync(distDir)) {
        ...
        if (ignored.code === 0)
            await runArgs('git', ['-C', projectDir, 'add', '-f', 'dist/'], { timeoutMs: 30_000 });
    }
    ...
    if (!pushDetail.startsWith('pushed'))
        return { status: 'error', step: 'git push', detail: pushDetail };
