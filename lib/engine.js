...
export function runArgs(cmd, args, opts = {}) {
    return new Promise((resolve) => {
        ...
        const push = (buf, target) => {
            if (target === 'out') {
                ...
                if (out.length > 2_000_000) {
                    out = out.slice(0, 2_000_000);
                    truncated = true;
                }
            }
            else {
                err += buf;
                if (err.length > 2_000_000) {
                    err = err.slice(0, 2_000_000);
                    truncated = true;
                }
            }
        };
        ...
        child.on('close', (code) => { if (t)
            clearTimeout(t); resolve({ code, stdout: truncated ? out + '\n[truncated]' : out, stderr: err }); });
        child.on('error', (e) => { if (t)
            clearTimeout(t); resolve({ code: -1, stdout: out, stderr: String(e) }); });
    });
}
export function readStartScript(projectDir) {
...
// ---------------------------------------------------------------------------
export async function waitHealthy(port, timeoutMs = 40_000) {
        ...
        for (const p of paths) {
            try {
                ...
                if (r.status >= 200 && r.status < 300)
                    return { ok: true, detail: `${p} -> ${r.status}` };
            }
            catch { /* keep polling */ }
        }
        await sleep(1500);
...
export async function waitTunnelUrl(getLogs, timeoutMs = 45_000) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
...
export function directHttpsProbe(hostname, ip, pathname) {
    return new Promise((resolve) => {
        const req = https.get({
            host: ip,
            servername: hostname,
...
export async function verifyPublic(url) {
    let lastErr;
    for (let i = 1; i <= 5; i++) {
        try {
    ...
    try {
        ...
        const addr = await Promise.race([
            ...
            new Promise((_, rej) => setTimeout(() => rej(new Error('DNS resolve timeout')), 8000)),
        ]);
        const ip = addr[0];
        ...
        return {
            ...
            detail: `本地 DNS 失败(${lastErr?.cause ? String(lastErr.cause.code) : lastErr?.message})；8.8.8.8 直连探测 ${ok ? '正常' : '失败'}@${ip}`,
            directOk: ok,
            ip,
        };
    }
    catch (e2) {
        return { ok: false, detail: `fetch failed: ${lastErr?.message}; 直连兜底也失败: ${e2.message}` };
    }
}
// ---------------------------------------------------------------------------
...
async function ghPostRetry(url, params) {
    let last;
    for (let i = 1; i <= 4; i++) {
        try {
            return await ghPost(url, params);
        }
        catch (e) {
            last = e;
            await sleep(3000 * i);
...
export async function requestDeviceCode() {
    try {
        ...
        flow.promise = (async () => {
            ...
            while (Date.now() < deadline) {
                ...
                try {
                    ...
                    appendLog('poll ' + n + ' hard-fail, continuing');
                    continue;
                }
                if (t.access_token) {
                    try {
                        ...
                        appendLog('token write failed: ' + e.message);
                        return 'error';
                    }
                    return 'ok';
                }
                if (t.error === 'authorization_pending') {
                    appendLog('poll ' + n + ': waiting for user...');
                    continue;
                }
        ...
        return { error: `申请设备码失败: ${e.message}` };
    }
}
function appendLog(msg) {
    try {
        appendFileSync(POLL_LOG, `${new Date().toISOString()} ${msg}\n`);
    }
    catch { /* noop */ }
}
...
export async function finishGhAuth(flow) {
    if (!existsSync(flow.tokenFile))
        return { ok: false, detail: 'no token yet' };
    ...
    try {
        rmSync(flow.flowDir, { recursive: true, force: true });
    }
    catch { /* noop */ }
    if (r.code === 0)
...
export async function ensureGhAuthBlocking() {
    if (await ghAuthed())
        return { ok: true, detail: 'gh already authenticated' };
    ...
    if (!fin.ok)
        return { ok: false, detail: fin.detail };
    return { ok: true, detail: 'gh authenticated' };
}
// ---------------------------------------------------------------------------
...
async function unstageSecrets(projectDir) {
...
export async function publishToGithub(projectDir, opts) {
    ...
    if (isRepo.code !== 0) {
        ...
        if (init.code !== 0)
            return { status: 'error', step: 'git init', detail: init.stderr.trim() || init.stdout.trim() };
    }
    const hasName = await runArgs('git', ['-C', projectDir, 'config', 'user.name'], { timeoutMs: 10_000 });
    if (hasName.code !== 0)
        await runArgs('git', ['-C', projectDir, 'config', 'user.name', 'dsh-deploy'], { timeoutMs: 10_000 });
    const hasEmail = await runArgs('git', ['-C', projectDir, 'config', 'user.email'], { timeoutMs: 10_000 });
    if (hasEmail.code !== 0)
        await runArgs('git', ['-C', projectDir, 'config', 'user.email', 'dsh-deploy@users.noreply.github.com'], { timeoutMs: 10_000 });
    ...
    if (includeDist && existsSync(distDir)) {
        ...
        if (ignored.code === 0) {
            ...
            if (add.code !== 0)
                return { status: 'error', step: 'git add -f dist', detail: add.stderr.trim() };
        }
    }
    const addAll = await runArgs('git', ['-C', projectDir, 'add', '-A'], { timeoutMs: 30_000 });
    if (addAll.code !== 0)
        return { status: 'error', step: 'git add', detail: addAll.stderr.trim() };
    // 3) 秘密文件拦截（自动撤出 + 明确报告）
    ...
    if (commit.code !== 0)
        return { status: 'error', step: 'git commit', detail: commit.stderr.trim(), staged: stagedSummary };
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        if (p.code === 0) {
            pushDetail = `pushed on attempt ${i}`;
            break;
        }
        pushDetail = `attempt ${i} failed: ${p.stderr.trim().split('\n').slice(-2).join(' ')}`;
        await sleep(3000 * i);
    }
    if (!pushDetail.startsWith('pushed'))
        return { status: 'error', step: 'git push', detail: pushDetail, staged: stagedSummary };
...
export function spawnDetached(cmdLine, cwd, logFile, env) {
...
export function tailFile(file, n = 30) {
    try {
...
export async function doctor(projectDir) {
    ...
    try {
        ...
        push('网络/DNS', false, `trycloudflare.com 不可达: ${e.message}（隧道链接可能需公共 DNS/安全 DNS/手机流量）`);
    }
